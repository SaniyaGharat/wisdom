...
export function AdminDashboard() {
  ...
  const rawItems = categories.data && "items" in categories.data && Array.isArray(categories.data.items) ? categories.data.items : undefined;
  const bars: Array<{ name: string; count: number }> = categories.data ? (rawItems ? rawItems.map((item: Record<string, unknown>) => ({ name: value(item["category"] ?? item["name"]), count: Number(item["count"] ?? item["total"] ?? 0) })) : Object.entries(categories.data).map(([name, count]) => ({ name, count: Number(count) }))) : [];
  const max = Math.max(...bars.map((item) => item.count), 1);
  async function runAll() { setRunning(true); try { await api.runAllMatching(); toast.success("Platform-wide matching complete"); await Promise.all([summary.refetch(), matches.refetch(), activity.refetch()]); } catch (error) { toast.error(error instanceof Error ? error.message : "Matching failed"); } finally { setRunning(false); } }
