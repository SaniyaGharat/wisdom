...
export function MatchDashboard({ kind, id }: { kind: "client" | "supplier"; id: string }) {
  ...
  return <div className="page-wrap">
    ...
    <section className="requirement-strip"><div><p className="text-xs font-bold uppercase text-muted-foreground">Your {kind === "client" ? "requirement" : "offer"}</p><p className="mt-2 max-w-3xl font-display text-xl">{product}</p></div><div className="grid gap-1 text-sm"><span><strong>Category:</strong> {text(profile["category"])}</span><span><strong>Location:</strong> {text(profile["location"])}</span></div></section>
...
function MatchCard({ match, rank, counterpart, busy, onStatus }: { match: Match; rank: number; counterpart: "client" | "supplier"; busy: boolean; onStatus: (status: string) => void }) {
  ...
  return <article className="match-card"><div className="flex flex-col gap-5 md:flex-row md:items-start"><div className="flex flex-1 gap-4"><span className="rank-mark">{rank}</span><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-display text-xl font-semibold">{name}</h3><span className="status-badge">{currentStatus}</span></div><p className="mt-1 text-sm font-medium text-primary">{category}</p><p className="mt-2 flex items-center gap-1 text-sm text-muted-foreground"><MapPin className="size-4" />{location}</p><p className="mt-4 max-w-3xl text-sm leading-6 text-muted-foreground">{text(match.match_reason ?? match.reason, "This profile shares relevant requirements and capabilities with yours.")}</p></div></div><div className={`score-badge ${tone}`}><strong>{Math.round(score)}%</strong><span>match</span></div></div>
