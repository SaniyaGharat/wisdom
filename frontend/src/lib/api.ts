...
export const api = {
  ...
  getClients: (params?: URLSearchParams) => request<ListEnvelope<Record<string, unknown>>>(`/api/clients${params ? `?${params.toString()}` : ""}`),
  getSuppliers: (params?: URLSearchParams) => request<ListEnvelope<Record<string, unknown>>>(`/api/suppliers${params ? `?${params.toString()}` : ""}`),
